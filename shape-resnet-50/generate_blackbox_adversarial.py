"""Generate black-box adversarial images
"""
import argparse
...
adv_save_dir = args.base_dir + 'blackbox_adversarial/l2/'
...
fmodel = PyTorchModel(model, bounds=(0, 1), num_classes=n_classes, preprocessing=preprocessing)
...
for val_batch in range(num_batches):
    ...
    for img_i in range(num_imgs_per_batch):
        ...
        attack = RandomStartProjectedGradientDescentAttack(model=fmodel, criterion=TargetClass(target_label),
                                                           distance=Ltwo)
        adversarial = attack(image, clean_label, binary_search=False, epsilon=epsilon, stepsize=2./255, iterations=10,
                             random_start=True)
